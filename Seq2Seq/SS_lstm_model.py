...
#             # 🔥 Flush so TensorBoard updates live
#             writer.flush()
...
class ModelTrainer:
    def __init__(self, input_size, hidden_size=32, num_layers=2, lr=1e-3,
                 batch_size=16, num_epochs=50, loss_fn="mse", dropout=0.5, device=None):
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lr = lr
        ...
        class LSTMSeq2Seq(nn.Module):
            def __init__(self, input_size, hidden_size, num_layers, dropout):
                ...
                self.fc = nn.Sequential(
                    ...
                    nn.Linear(hidden_size, hidden_size // 2),
                    nn.ReLU(),
                    nn.Dropout(0.5),   # 🔼 added dropout before output
                    nn.Linear(hidden_size // 2, 1)
                )
    ...
    #     optimizer = optim.Adam(self.model.parameters(), lr=self.lr, weight_decay=1e-4)  # 🔼 stronger L2
    #     self.model.train()

    #     writer = SummaryWriter(log_dir="runs/experiment_seq2seq")
    #     global_step = 0
    ...
    #                 loss_val = self.criterion(preds_val, y_val)
    ...
    def train(self):
        ...
        optimizer = optim.Adam(self.model.parameters(), lr=self.lr, weight_decay=1e-5)
        writer = SummaryWriter(log_dir="runs/experiment_seq2seq2")
        ...
        for epoch in range(self.num_epochs):
            ...
            # Metrics
            rmse = root_mean_squared_error(true_inv.flatten(), preds_inv.flatten())
            r2 = r2_score(true_inv.flatten(), preds_inv.flatten())
            corr = np.corrcoef(true_inv.flatten(), preds_inv.flatten())[0, 1]

            # ---- Logging ----
            print(f"Epoch [{epoch+1}/{self.num_epochs}], "
                  f"Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}, "
                  f"RMSE: {rmse:.4f}, R²: {r2:.4f}, Corr: {corr:.4f}")

            writer.add_scalar("Loss/train_epoch", avg_train_loss, epoch)
            writer.add_scalar("Loss/val_epoch", avg_val_loss, epoch)
            writer.add_scalar("Metrics/RMSE", rmse, epoch)
            writer.add_scalar("Metrics/R2", r2, epoch)
            writer.add_scalar("Metrics/Correlation", corr, epoch)
