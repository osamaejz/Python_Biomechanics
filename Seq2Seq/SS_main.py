...
from SS_Data_process import Process
import numpy as np
from SS_lstm_model import ModelTrainer
...
for idx, (curr_EMG, curr_Kin) in enumerate(zip(EMG_Data, Kin_Data)):
    ...
    for flt in range(1, 17):
        env, filt, notch, rect = Process.preprocess_emg(curr_EMG[flt], fs=2000, target_fs=fs)
        envelope.append(env)
    ...
    emg_win, kin_win = Process.window_signals(emg_envelop, curr_Kin, fs=fs,
...
plt.plot(trues[:400], label='True')
plt.plot(preds[:400], label='Pred')
plt.legend()
plt.show()
